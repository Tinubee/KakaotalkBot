function DeepLearning(room, msg, sender, replier) {
    ...
    if (msg.startsWith('/블랙리스트')) {
        let list = [];
        for (let i in jsonblacklist[room]) list.push('· ' + i + ' == ' + jsonblacklist[room][i]);
        replier.reply('► 블랙리스트: ' + Object.keys(jsonblacklist[room]).length + '명' + Lw + line + list.join('\n\n'));
        return;
    }
    if (msg.startsWith('/관리자리스트')) {
        let list = [];
        for (let i in jsonadmin[room]) list.push('· ' + i + ' == ' + jsonadmin[room][i]);
        replier.reply('► 관리자: ' + Object.keys(jsonadmin[room]).length + '명' + Lw + line + list.join('\n\n'));
        return;
    }
    if (msg.startsWith('/금지어리스트')) {
        let list = [];
        for (let i in jsonBenWord[room]) list.push('· ' + i + ' == ' + jsonBenWord[room][i]);
        replier.reply('► 금지어: ' + Object.keys(jsonBenWord[room]).length + '개' + Lw + line + list.join('\n\n'));
        return;
    }
    ...
    if (msg.startsWith('/블랙추가')) {
        if (Object.keys(jsonadmin[room]).includes(sender).valueOf() || sender == "김형민") {
            let a = msg.substr(5).trim();
            if (Object.keys(jsonadmin[room]).includes(a).valueOf() || a.includes("김형민")) {
            ...
            replier.reply(sender + '님이 ' + a + "님을 블랙리스트로 등록 시켰습니다. 현재블랙리스트 : " + blackcount + "명");
            return;
        }
        else {
    ...
    if (msg.startsWith('/관리자등록')) {
        if (Object.keys(jsonadmin[room]).includes(sender).valueOf() || sender == "김형민") {
            ...
            replier.reply(sender + '님이 ' + a + "님을 관리자로 등록 시켰습니다. 현재관리자 : " + admincount + "명");
            return;
        }
        else {
            replier.reply(sender + '님은 관리자가 아닙니다.');
    ...
    if (msg.startsWith('/블랙삭제')) {
        if (Object.keys(jsonadmin[room]).includes(sender).valueOf() || sender == "김형민") {
            ...
            replier.reply(msg.substr(5).trim() + '님을 성공적으로 삭제하였습니다. 현재블랙리스트 : ' + blackount + "명");
            return;
        }
        else {
            replier.reply(sender + '님은 관리자가 아닙니다.');
    ...
    if (msg.startsWith('/관리자삭제')) {
        if (sender == "김형민") {
            ...
            fs.write(pathadmin, JSON.stringify(jsonadmin, null, 4));
            let admincount = Object.keys(jsonadmin[room]).length;
            replier.reply('성공적으로 삭제하였습니다. 현재관리자 : ' + admincount + "명");
            return;
        }
        else {
    ...
    if (msg.startsWith('/제시어추가')) {
        ...
        if (msg.split('==').length < 2) {
            replier.reply('[/가르치기 등록된 제시어(대표1개) == 추가할 제시어] 형식으로 작성해 주세요\n► ex) /제시어추가 안녕 == 하이');
            return;
        }
        let a = msg.substr(6).split('==')[0].trim();
        let b = msg.substr(msg.split('==')[0].length + 2).trim();
        if (b == '') {
            replier.reply('추가할 제시어를 써주세요! \n► ex) /제시어추가 ' + a + ' == 테스트');
            return;
        }
        ...
        let Findkey = Object.keys(jsondb[room]).filter(function (find) { return find.indexOf(a) > -1; });
        if (Findkey[0] != undefined) {
            ...
            for (h = 0; h < sl_fk.length; h++) {
                for (m = 0; m < split_b.length; m++) {
                    if (split_b[m] == sl_fk[h]) {
                        replier.reply('이미 등록된 제시어가 있습니다!\n► 중복등록 제시어 :' + split_b[m]);
                        return;
                    }
                }
            }
            ...
            replier.reply('[' + b + '] 라는 제시어가 [' + Findkey[0] + '] 에 추가됐습니다.');
            return;
    ...
    if (msg.startsWith('/답장삭제')) {
        if (Object.keys(jsonblacklist[room]).includes(sender).valueOf() == false) {
            if (msg.split('==').length < 2) {
                replier.reply('[/답장삭제 반응할 말 == 대답] 형식으로 작성해 주세요\n► ex) /답장삭제 안녕 == 만나서 반가워:)');
                return;
            }
            let a = msg.substr(5).split('==')[0].trim();
            let b = msg.substr(msg.split('==')[0].length + 2).trim();
            if (b == '') {
                replier.reply('해당 말에 대한 삭제할 대답을 써주세요! \n► ex) /답장삭제 ' + a + ' == 테스트');
                return;
            }
